...
valid_l <- valid %>%
  ...
  select("flen_AN1":"rel_4_sr", "id", "semester") %>%
  gather(key = "variable", value = "value", na.rm = T, 1:51) %>%                       # create long data set
...
valid_cfa <- valid_l %>%
  filter(!is.na(treat) & variable != "tc") %>%
...
# # test for measurment invariance: model without restrictions on loadings
# f4.model2.2 <- 'level: 1
...
#              '
# 
# fit_f4.2.2 <- sem(f4.model2.2, 
#               data = valid_cfa, 
#               cluster = "id",
#               std.lv=TRUE
#               )
# 
# anova(fit_f4.2, fit_f4.2.2)





# # test for measurement invariance | PENDING
# library(semTools)
# mi_f4 <- measEq.syntax(configural.model = f4.model2,      # ERROR SAMUEL
#                        data = valid_cfa,
#                        group = "treat",
#                        cluster = "id")
```
...
# compute BF
library(BayesFactor)
ttestBF(x = pred_exp$transfer_w[pred_exp$treat == 0], y = pred_exp$transfer_w[pred_exp$treat == 1],
        paired = T,
        nullInterval = c(-Inf, .5)
        )
```
\
\
...
pred_con <- data.frame(lavPredict(fit_con, method = "ML", level = 2))

correlationBF(y = pred_con$transfer_b, x = pred_con$umsetzung, 
              #rscale = .3,                                   # width of prior: standard
              nullInterval = c(-1, .3)                        # assumption was medium effect
              )

correlationBF(y = pred_con$transfer_b, x = pred_con$differenz, 
              # rscale = .3,                                    # width of prior: standard
              nullInterval = c(0, 1)                          # assumption was negative effect
              )
